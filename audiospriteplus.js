...
function appendFile(name, src, dest, cb) {
  ...
  require('util').pump(reader, writer, function() {
    ...
    , loop: name === argv.autoplay
     ...
     json.howlersprite.sprite[name] = []
    json.howlersprite.sprite[name].push(offsetCursor)
    json.howlersprite.sprite[name].push(offsetCursor + duration)
...
function processFiles() {
  ...
  , m4a: []
  //, ogg: '-acodec libvorbis -f ogg'.split(' ') // Removed libvorbis for now since I can't get it to build into FFMPEG properly.
  , ogg: '-f ogg'.split(' ')
  }
